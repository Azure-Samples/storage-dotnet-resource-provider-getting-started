    ...
    public class StorageAccountTests
    {
        ...
        static async Task Main()
        {
            ...
            try
            {
                ...
                ArmOperation<StorageAccountResource> accountCreateOperation = await accountCollection.CreateOrUpdateAsync(WaitUntil.Completed, storAccountName, parameters);
                ...
                GetStorageAccountsInResourceGroup(resourceGroup);

                //Get all the storage accounts for a given subscription
                GetStorageAccountsForSubscription(subscription);
        ...
        /// <summary>
        private static void GetStorageAccountsInResourceGroup(ResourceGroupResource resourceGroup)
        {
            IEnumerable<StorageAccountResource> storAccts = resourceGroup.GetStorageAccounts().GetAll();
            Console.WriteLine($"List of storage accounts in {resourceGroup.Id.Name}:");
            foreach (StorageAccountResource storAcct in storAccts)
            {
                Console.WriteLine($"\t{storAcct.Id.Name}");
            }
        }

        private static void GetStorageAccountsForSubscription(SubscriptionResource subscription)
        {
            IEnumerable<StorageAccountResource> storAcctsSub = subscription.GetStorageAccounts();
            Console.WriteLine($"List of storage accounts in subscription {subscription.Get().Value.Data.DisplayName}:");
            foreach (StorageAccountResource storAcctSub in storAcctsSub)
            {
